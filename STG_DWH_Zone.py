...
google_cloud_options.job_name = "dataflowdwzone"
...
source_table_spec = bigquery.TableReference(
    projectId="studied-client-307710", datasetId="SMT_DWH", tableId="SecondAxis"
)
sink_table_spec = bigquery.TableReference(
    projectId="studied-client-307710", datasetId="SMT_DWH", tableId="SMT_REF_Zone"
)

def make_sink_schema():
    mapping = {
        "CORE_Id_ZoneId":          "INTEGER", 
        "CORE_LB_ZoneParentCode":    "STRING",
        "CORE_LB_Level":  "INTEGER", 
        "CORE_LB_ZoneCode":           "STRING", 
        "CORE_LB_ZoneEntityEN":     "STRING",
...
class ElementCleanup(beam.DoFn):
    ...
    def make_transform_map(self):
        return {
            "CORE_Id_ZoneId": [],
            "CORE_LB_ZoneParentCode":    [self.trim],
            "CORE_LB_Level":     [self.trim, self.percent_cleaner, self.to_int],
            "CORE_LB_ZoneCode":  [self.trim],
            "CORE_LB_ZoneEntityEN":     [self.trim],
            "CORE_LB_ZoneEntityFR":        [self.trim],
            "CORE_LB_ZoneEntityStatus":          [self.trim],
            "CORE_LB_Order":      [self.trim, self.percent_cleaner, self.to_int], #keeping encoding as "LAT,LON" as data studio likes this
            "CORE_ID_ZoneSourceId":      [self.trim, self.percent_cleaner, self.to_int], #keeping encoding as "LAT,LON" as data studio likes this
            "CORE_DT_LastMod": [self.trim, self.to_datetime],
            "CORE_DT_RecordCreationDate": [self.passe],
            "CORE_DT_RecordModificationDate": [self.passe],
            "CORE_FL_IsDeleted": [self.passe],
            "CORE_FL_Latitude": [self.passe],
            "CORE_FL_Longitude": [self.passe]
        }
