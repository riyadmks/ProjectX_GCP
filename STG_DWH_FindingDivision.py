...
sink_table_spec = bigquery.TableReference(
    projectId="studied-client-307710", datasetId="SMT_DWH", tableId="SMT_REF_FindingDivision"
)


source = BigQuerySource(query="SELECT distinct fin.CORE_ID_FindingId as CORE_ID_FindingId, div.CORE_ID_DivisionId as CORE_ID_DivisionId, id as CORE_ID_FindingSourceId, current_date() as CORE_DT_RecordCreationDate, current_date() as CORE_DT_RecordModificationDate, 0 as CORE_FL_IsDeleted FROM `studied-client-307710.SMT_STG.CS_FindingsExportImport` imp LEFT OUTER JOIN   `studied-client-307710.SMT_DWH.SMT_REF_Finding` fin ON   CAST(imp.Id AS int64) = fin.CORE_ID_FindingSourceId LEFT OUTER JOIN  `studied-client-307710.SMT_DWH.SMT_REF_Division` div ON CAST(imp.CS_ConcernedDivisions AS int64) = div.CORE_ID_DivisionSourceId", use_standard_sql=True)  # you can also use SQL queries
...
class ElementCleanup(beam.DoFn):
    ...
    def make_transform_map(self):
        return {
            ...
            "CORE_ID_DivisionId":    [self.trim, self.percent_cleaner, self.to_int, self.NulltoNUM],
