...
google_cloud_options.job_name = "dataflowdwrisk"
...
source_table_spec = bigquery.TableReference(
    projectId="studied-client-307710", datasetId="SMT_DWH", tableId="Risks"
)
sink_table_spec = bigquery.TableReference(
    projectId="studied-client-307710", datasetId="SMT_DWH", tableId="SMT_REF_Risk"
)


source = BigQuerySource(query="SELECT ROW_NUMBER() over(order by Parent) as CORE_ID_RiskId, Parent as CORE_LB_RiskParentReference, Level  as CORE_LB_RiskLevel, Ref as CORE_LB_RiskReference, `Order` as CORE_LB_Order, Active as CORE_LB_Active, NameEN as CORE_LB_RiskNameEN, NameFR as CORE_LB_RiskNameFR, DefEN as CORE_LB_RiskDefEN, DefFR as CORE_LB_RiskDefFR, KeyRisk as CORE_LB_KeyRisk, NonConformityRisk as CORE_LB_NonConformityRisk, IdentificationDate as CORE_DT_IdentificationDate, Id as CORE_ID_RiskSourceId, LastMod as CORE_DT_LastModif, current_date() as CORE_DT_RecordCreationDate, current_date() as CORE_DT_RecordModificationDate, 0 as CORE_FL_IsDeleted FROM `studied-client-307710.SMT_STG.Risks`", use_standard_sql=True)  # you can also use SQL queries
...
class ElementCleanup(beam.DoFn):
    ...
    def make_transform_map(self):
        return {
            "CORE_ID_RiskId": [self.passe],
            "CORE_LB_RiskParentReference":    [self.trim],
            "CORE_LB_RiskLevel":     [self.trim, self.percent_cleaner, self.to_int],
            "CORE_LB_RiskReference":  [self.trim],
            "CORE_LB_Order":     [self.trim, self.percent_cleaner, self.to_int],
            "CORE_LB_Active":        [self.trim],
            "CORE_LB_RiskNameEN":          [self.trim],
            "CORE_LB_RiskNameFR":      [self.trim], 
            "CORE_LB_RiskDefEN":      [self.trim], 
            "CORE_LB_RiskDefFR": [self.trim],
            "CORE_LB_KeyRisk": [self.trim],
            "CORE_LB_NonConformityRisk": [self.trim],
            "CORE_DT_IdentificationDate": [self.to_datetime],
            "CORE_ID_RiskSourceId": [self.trim, self.percent_cleaner, self.to_int],
