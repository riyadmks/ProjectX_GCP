...
google_cloud_options.job_name = "dataflowdwhauditprocess"
...
sink_table_spec = bigquery.TableReference(
    projectId="studied-client-307710", datasetId="SMT_DWH", tableId="SMT_REF_AuditProcess"
)


source = BigQuerySource(query="SELECT distinct 1 as CORE_ID_AuditId, CORE_ID_ProcessId as CORE_ID_ProcessId, current_date() as CORE_DT_RecordCreationDate, current_date() as CORE_DT_RecordModificationDate, 0 as CORE_FL_IsDeleted FROM `studied-client-307710.SMT_STG.Missions` miss left outer join `studied-client-307710.SMT_DWH.SMT_REF_Process` process on cast(miss.AuditedProcesses as INT64) = process.CORE_ID_ProcessSourceId", use_standard_sql=True)  # you can also use SQL queries
...
class ElementCleanup(beam.DoFn):
    ...
    def make_transform_map(self):
        return {
            ...
            "CORE_ID_ProcessId":    [self.trim, self.percent_cleaner, self.to_int, self.NulltoNA],
