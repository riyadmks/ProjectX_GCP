...
source_table_spec = bigquery.TableReference(
    projectId="studied-client-307710", datasetId="SMT_STG", tableId="RecommendationCriticity"
)
sink_table_spec = bigquery.TableReference(
    projectId="studied-client-307710", datasetId="SMT_DWH", tableId="SMT_REF_RiskLevel"
)
...
class ElementCleanup(beam.DoFn):
    ...
    def make_transform_map(self):
        return {
            "CORE_ID_RiskLevelId": [self.passe],
            "CORE_LB_RiskLevelCode":    [self.trim],
            "CORE_LB_RiskLevelNameEN":     [self.trim],
            "CORE_LB_RiskLevelNameFR":  [self.trim],
            "CORE_LB_RiskLevelDescriptionEN":     [self.trim],
            "CORE_LB_RiskLevelDescriptionFR":        [self.trim],
            "CORE_LB_Active":          [self.trim],
            "CORE_LB_Order":      [self.trim, self.percent_cleaner, self.to_int], 
            "CORE_LB_TextColor":      [self.trim], 
            "CORE_LB_BackgroundColor": [self.trim],
			...
			"CORE_ID_RisklLevelSourceId": [self.trim, self.percent_cleaner, self.to_int],
